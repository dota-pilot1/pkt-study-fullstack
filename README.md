...
FDE라는 직군이 부상하고 AX가 확산될수록, 개발자의 기본기는 오히려 더 중요해집니다. AI가 구현의 상당 부분을 맡을 수는 있어도 **무엇을 만들어야 하는지, 기능을 어떻게 나눌지, 어떤 순서로 진행할지, 무엇을 기준으로 검증할지**는 결국 개발자가 판단해야 하기 때문입니다.

티키타카 개발 노트는 바이브 코더에서 Agent Engineer로 성장하는 데 필요한 다음 세 가지 역량을 실제 작업 안에서 훈련하기 위해 만들었습니다.

### 1. 개발 개념을 퍼즐처럼 다루는 능력

언어, 데이터베이스, 네트워크, 인증, 상태 관리 같은 핵심 개념을 이해하고, 문제를 작은 기술 요소로 분해한 뒤 다시 조합합니다. Agent에게 작업을 맡기기 전에 어떤 개념과 제약이 필요한지 설명할 수 있어야 합니다.

→ **개념 이해력**

### 2. 기능을 모듈처럼 다루는 능력

DDD, FSD, Clean Architecture, SOLID, TDD를 바탕으로 기능의 경계를 나누고, 한 번에 큰 구현을 맡기기보다 부분별로 구현하고 검증합니다. 이렇게 나눈 작업은 Agent에게도 더 명확한 지시가 되고, 리뷰 범위도 작고 정확해집니다.

→ **모듈화·부분 검증 능력**

### 3. 설계 의도를 주도적으로 관리하는 능력

기능의 목적, 책임, 제약과 비즈니스 요구를 이해하고 관리합니다. Agent가 구현을 맡더라도 전체 설계 방향과 핵심 의도는 사람이 유지해야 합니다. 그래야 구현 결과를 받아들이거나 수정할 기준이 생깁니다.

→ **설계 의도 관리 능력**

### 계획·문서화로 세 역량을 실행하기

앞으로는 특히 **계획을 세우는 능력**이 중요합니다. 단순히 “이 기능 만들어줘”라고 요청하는 대신, 목표·범위·우선순위·완료 조건을 정한 계획을 세우고 문서화해야 합니다. Agent를 잘 활용하는 일은 먼저 문제를 구조화하고 작업을 나눈 뒤, 그 의도를 AI에게 명확히 전달하는 능력과 연결됩니다.

코드뿐 아니라 개발 과정과 판단을 문서화하는 능력도 중요해집니다. 막연히 뛰어난 개발자가 되기를 기대하기보다 다음처럼 Bottom-up 방식으로 역량을 쌓는 편이 현실적입니다.

```text
기본 개념 → 작은 실습 → 계획 수립 → 실제 프로젝트 적용 → Agent 활용 → 코드 리뷰 → 문서화
```

강의를 많이 듣다 보면 지식이 한꺼번에 쏟아져 머릿속에 홍수가 나는 듯할 수 있습니다. 그래서 피자나 조각 케이크처럼 학습 내용을 작은 단위로 나누고, 아래 흐름으로 다시 구조화할 필요가 있습니다.

```text
개념 → 작은 실습 → 실제 적용 → 실패 경험 → 리뷰 → 정리
```

AI를 컴팩트하게 활용하려면 내 머릿속에도 구조화된 지식이 있어야 합니다. AI에 좋은 Context를 제공하려면 먼저 내가 문제와 지식을 구조화할 수 있어야 하기 때문입니다.

AI에게 친절하게 말하거나, 화를 내거나, 하루 종일 대화를 많이 한다고 해서 Agent Engineer가 되는 것은 아닙니다. 개발에도 **덧셈·뺄셈 → 구구단 → 방정식 → 미분·적분**처럼 단계적으로 쌓아야 하는 기본기가 있습니다.

이 기본기가 부족한 상태에서 AI의 구현 능력에만 의존하면 다음과 같은 상태가 되기 쉽습니다.

- 기본기 없이 주먹부터 휘두르는 **길거리 막싸움꾼**
- 전술과 빌드업 없이 앞으로만 차는 **지독한 뻥축구**
- 수읽기와 기본 형태를 모르면서 감각으로만 두는 **바둑 기사**
- 물에 제대로 뜨지도 못하는 **수영 선수**
- 민첩성이 부족하고 조금만 흔들려도 자세가 쉽게 무너지는 **탁구 선수**

당장은 결과가 나오고 개발 속도도 빨라 보일 수 있습니다. 하지만 상황이 복잡해질수록 **왜 그렇게 만들어졌는지, 어디를 고쳐야 하는지, 무엇이 핵심인지** 스스로 판단하기 어려워집니다.

Agent Engineer가 되려면 AI라는 강력한 도구 이전에 **개발의 기본 근육, 구조를 보는 눈, 설계 의도를 붙잡는 힘**이 필요합니다. 그렇지 않으면 AI를 부리는 개발자가 아니라, **AI가 계속 만들어내는 코드와 문제를 뒤쫓는 개발자**가 될 수 있습니다.

그래서 만든 것이 **티키타카 개발 노트**입니다. 단순히 공부한 내용을 기록하는 앱이 아니라, 사이드 프로젝트와 엔터프라이즈 프로젝트에서 얻은 경험을 재사용 가능한 지식으로 축적하는 개발 계획·학습·지식 시스템입니다.

```text
할 일 정의 → 계획 → Agent 구현 → 검증·리뷰 → 개발 노트 → 다음 작업에 재사용
```

할 일에는 목표와 완료 기준을, 계획에는 작업 범위와 우선순위를, 구현 지시에는 API와 폴더를, 리뷰에는 검증 근거를, 노트에는 선택의 이유와 재사용할 지식을 남깁니다. 개발자는 문제를 이해하고 계획을 세우며 의도를 문서화하고, Agent에게 일을 나누고, 결과를 검증한 뒤 다시 지식으로 축적합니다.
