...
AI가 코드를 빠르게 만들어 주는 시대일수록, 사람에게는 구현을 요청하는 능력보다 **무엇을 만들고 왜 그렇게 나눌지 판단하는 능력**이 필요합니다. 티키타카 개발 노트는 바이브 코더에서 Agent Engineer로 성장하는 데 필요한 다음 세 가지 역량을 실제 작업 안에서 훈련하기 위해 만들었습니다.
...
티키타카 개발 노트는 이 세 역량이 일하는 흐름 안에서 연결되도록 만듭니다. 할 일에는 목표와 완료 기준을, 구현 지시에는 작업 범위와 API를, 리뷰에는 검증 근거를, 노트에는 선택의 이유와 재사용할 지식을 남깁니다.
