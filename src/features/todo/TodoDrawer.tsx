...
function TodoDetail({ todo, isClosing, onClose, onSave, onRefresh, onDelete }: { todo: TodoItem; isClosing: boolean; onClose: () => void; onSave: (patch: Partial<TodoItem>) => Promise<unknown>; onRefresh: () => Promise<unknown>; onDelete: () => Promise<boolean> }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const saved = await onSave({ title: normalizedTitle, description, checklist, verificationChecks });
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-5">
        <div className="mx-auto max-w-6xl space-y-5">
          <label className="block">
            <span className="mb-1 block text-[10px] font-black text-text-secondary">작업 제목</span>
            <input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="작업 제목" className="h-10 w-full rounded-lg border border-surface-border bg-surface-raised px-3 text-sm font-bold text-text-primary outline-none focus:border-brand-border" />
          </label>
          <label className="block">
            <span className="mb-1 block text-[10px] font-black text-text-secondary">작업 설명 <span className="font-medium text-text-muted">(선택)</span></span>
            <textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="구현 범위와 작업 목적" className="min-h-28 w-full resize-y rounded-lg border border-surface-border bg-surface-raised p-3 text-xs leading-5 text-text-primary outline-none focus:border-brand-border" />
          </label>
                  ...
                  return <SortableChecklistRow key={item.id} item={item} index={index}>
                    {({ ref, handleRef, isDragSource }) => (
                      <div ref={ref} className={`flex items-center gap-3 rounded-lg border border-surface-border-soft px-3 py-2.5 text-xs text-text-secondary hover:bg-surface-muted/50 ${isDragSource ? "opacity-55 ring-2 ring-brand-border/50" : ""}`}>
                        {isChecklistBulkEditing && <button type="button" ref={handleRef} title="드래그하여 계획 순서 변경" aria-label={`${item.text} 드래그`} className="grid size-5 shrink-0 cursor-grab touch-none place-items-center text-text-muted hover:text-brand-primary active:cursor-grabbing"><GripVertical className="size-4" /></button>}
          ...
          <div className="min-h-0 overflow-y-auto p-4 lg:border-r lg:border-surface-border-soft">
            <div className="overflow-hidden rounded-lg border border-surface-border-soft">
              ...
              <div>
                ...
                <InstructionSelectionGroup className="mt-2 border-t border-surface-border-soft pt-2" label="통과 조건 검증" items={passedVerificationChecks} selectedIds={selectedVerificationIds} onToggle={(id, selected) => toggleSelection(id, selected, setSelectedVerificationIds)} onToggleAll={(selected) => toggleAll(passedVerificationChecks, selected, setSelectedVerificationIds)} renderMeta={(item) => item.evidence ? "근거 있음" : "근거 없음"} />
              </div>
            </div>
          </div>
          <div className="flex min-h-0 flex-col gap-3 overflow-y-auto bg-surface-muted/20 p-4">
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-start justify-center bg-black/30 p-3 backdrop-blur-[1px] sm:p-5" onMouseDown={onClose}>
      <section className="mt-2 flex h-[calc(100vh-1.5rem)] w-full max-w-[1800px] flex-col overflow-hidden rounded-xl border border-surface-border bg-surface-raised shadow-2xl sm:mt-4 sm:h-[calc(100vh-2.5rem)]" onMouseDown={(event) => event.stopPropagation()}>
        <header className="flex items-center justify-between border-b border-surface-border-soft px-4 py-3">
          <div><p className="text-xs font-black text-brand-primary">{todo ? "세부 계획 요청 {}" : "Agent 작업 요청 {}"}</p><h3 className="text-sm font-black text-text-primary">{todo ? `${todo.title} · 상세 TODO API` : "NOVA 작업 TODO API"}</h3></div>
...
export function TodoDrawer({ open, onOpenChange, scope }: { open: boolean; onOpenChange: (open: boolean) => void; scope?: TodoScope }) {
  ...
  return createPortal(
    <div className={`fixed inset-0 z-[110] flex justify-end bg-black/20 backdrop-blur-[1px] ${visible ? "opacity-100" : "pointer-events-none opacity-0"}`} style={{ transition: "opacity 360ms ease-out" }} onMouseDown={() => {
      ...
      <aside role="dialog" aria-modal="true" aria-label="NOVA 작업 관리" onMouseDown={(event) => event.stopPropagation()} className="relative flex h-full flex-col border-l border-surface-border bg-surface-raised shadow-2xl will-change-transform" style={{ width: `${drawerSize}vw`, maxWidth: "none", transform: visible ? "translateX(0)" : "translateX(100%)", transition: "transform 400ms cubic-bezier(0.22, 1, 0.36, 1), width 240ms ease" }}>
        ...
        <header className="flex shrink-0 items-center justify-between border-b border-surface-border-soft px-4 py-3">
          ...
          <div className="flex items-center gap-1.5"><button type="button" onClick={() => void refreshTodos()} disabled={isRefreshing} title="작업 새로고침" aria-label="작업 새로고침" className="ui-icon-button size-8 text-text-muted hover:text-brand-primary disabled:cursor-wait disabled:opacity-50"><RefreshCw className={`size-3.5 ${isRefreshing ? "refresh-icon-spin" : ""}`} /></button><button type="button" onClick={() => setApiSpecOpen(true)} title="현재 주제 API Spec" className="inline-flex h-8 items-center justify-center rounded-lg border border-surface-border bg-surface-muted px-3 text-xs font-black text-text-secondary transition-colors hover:border-brand-border hover:text-brand-primary">API Spec</button><button type="button" onClick={() => setAgentGuideOpen(true)} title="Agent 작업 요청 안내" className="inline-flex h-8 items-center justify-center rounded-lg border border-brand-border bg-brand-glass px-3 text-xs font-black text-brand-primary transition-colors hover:bg-brand-primary/10">Agent 작업 요청 {"{}"}</button><button type="button" onClick={close} aria-label="닫기" className="ui-icon-button size-8 text-text-muted"><X className="size-4" /></button></div>
        </header>
        ...
        <div className="min-h-0 flex-1 overflow-auto p-3">
          ...
          <div className="mx-auto w-full max-w-[1280px] overflow-hidden rounded-lg border border-surface-border-soft">
            <div className="grid gap-2 bg-surface-muted px-3 py-2 text-[10px] font-black text-text-muted" style={{ gridTemplateColumns: "32px minmax(250px, 1fr) 84px 42px 96px" }}>
              ...
              <span>작업</span><span>상태</span><span className="text-center">중요</span><span className="text-right">관리</span>
            </div>
                  ...
                  <SortableTodoRow key={todo.id} todo={todo} index={index} disabled={reorderDisabled}>
                    {({ ref, handleRef, isDragSource, isDropTarget }) => (
                      <div ref={ref} className={`border-t border-surface-border-soft first:border-t-0 transition-[background-color,box-shadow,opacity] duration-200 ${isDragSource ? "z-10 scale-[1.01] opacity-55 shadow-xl ring-2 ring-brand-border/50" : ""} ${isDropTarget && !isDragSource ? "bg-brand-glass ring-2 ring-brand-border/70" : ""}`}>
                        <div className={`grid cursor-pointer items-center gap-2 px-3 py-2.5 hover:bg-surface-muted/60 ${selectedTodoId === todo.id ? "bg-brand-glass/30" : ""}`} style={{ gridTemplateColumns: "32px minmax(250px, 1fr) 84px 42px 96px" }} onClick={() => selectTodo(todo.id)}>
                          ...
                          <button type="button" onClick={(event) => { event.stopPropagation(); void toggleImportant(todo.id); }} className={`ui-icon-button mx-auto size-7 ${todo.important ? "text-amber-500" : "text-text-muted"}`}><Star className="size-3.5" fill={todo.important ? "currentColor" : "none"} /></button>
                          <div className="flex justify-end gap-0.5"><button type="button" onClick={(event) => { event.stopPropagation(); selectTodo(todo.id); }} className="ui-icon-button size-7 text-text-muted hover:text-brand-primary" title="작업 수정" aria-label={`${todo.title} 수정`}><Pencil className="size-3.5" /></button><button type="button" onClick={(event) => { event.stopPropagation(); void toggleComplete(todo.id); }} className="ui-icon-button size-7 text-text-muted" title={todo.status === "DONE" ? "미완료" : "완료"}>{todo.status === "DONE" ? <Check className="size-3.5 text-emerald-600" /> : <Circle className="size-3.5" />}</button><button type="button" onClick={(event) => { event.stopPropagation(); void deleteTodo(todo.id); }} className="ui-icon-button size-7 text-text-muted hover:text-destructive" title="삭제"><Trash2 className="size-3.5" /></button></div>
                        </div>
                      </div>
                    )}
                  </SortableTodoRow>
