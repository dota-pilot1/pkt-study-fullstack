...
export default function ImplementationNoteSamplePreview({ minHeight = "620px", selectedKeys, onSelectedKeysChange, showInlinePreview = true }: ImplementationNoteSamplePreviewProps) {
  ...
  const remove = useMutation({
    ...
    onSuccess: async (_, deletedKey) => {
      ...
      if (selectedKeys === undefined) setLocalSelectedKeys(nextSelected);
      onSelectedKeysChange?.(nextSelected);
      if (previewKey === deletedKey) setPreviewKey(null);
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["hospital-playbook", "samples"] }),
  ...
  const preview = (item: PlaybookSampleSummary) => {
    ...
    if (!showInlinePreview) setPreviewDialogOpen(true);
  };
  ...
  return (
    <div className="flex min-h-0 flex-col">
      <div className="shrink-0 border-b border-surface-border-soft bg-surface-raised px-4 py-3">
        <div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold leading-5 text-text-muted">샘플은 선택하면 지시문에 포함됩니다. 각 행의 미리 보기에서 본문을 확인하세요.</p><div className="flex shrink-0 items-center gap-2"><button type="button" onClick={() => void copySelectedSamples()} className="ui-icon-button-brand h-8 gap-1.5 px-2.5 text-[11px] font-black">{selectedSamplesCopied ? <Check className="size-3.5" /> : <Copy className="size-3.5" />}선택 샘플 복사 ({sampleItems.filter((item) => activeSelectedKeys.includes(item.sampleKey)).length})</button><button type="button" onClick={() => setAdding((current) => !current)} className="ui-icon-button-brand h-8 px-2.5 text-[11px] font-black">{adding ? "닫기" : "+ 예제"}</button></div></div>
        ...
        {samples.isLoading ? <p className="mt-3 text-xs font-bold text-text-muted">예제 목록을 불러오는 중입니다.</p> : samples.isError ? <p role="alert" className="mt-3 text-xs font-bold text-destructive">예제 목록을 불러오지 못했습니다.</p> : (
          <div className="mt-3 overflow-x-auto rounded-lg border border-surface-border-soft">
            <table className="w-full min-w-[720px] border-collapse text-left text-xs">
              <thead className="bg-surface-muted text-text-secondary"><tr><th className="w-12 px-3 py-2.5 text-center font-black"><input type="checkbox" checked={allSamplesSelected} onChange={toggleAllSamples} aria-label="모든 샘플 선택" className="size-4 accent-brand-primary" /></th><th className="px-3 py-2.5 font-black">예제</th><th className="min-w-48 px-3 py-2.5 font-black">조회 URL</th><th className="w-52 px-3 py-2.5 text-center font-black">관리</th></tr></thead>
              <tbody>{samples.data?.map((item) => <tr key={item.sampleKey} onClick={showInlinePreview ? () => preview(item) : undefined} className={`${showInlinePreview ? "cursor-pointer " : ""}border-t border-surface-border-soft transition-colors ${showInlinePreview && sampleKey === item.sampleKey ? "bg-brand-primary/5 hover:bg-brand-primary/10" : "hover:bg-surface-muted/60"}`}>
                ...
                <td className="px-3 py-3"><p className="font-black text-text-primary">{item.title}</p><p className="mt-1 font-mono text-[10px] text-text-muted">{item.sampleKey}</p></td>
                ...
                <td className="!align-middle px-3 py-3 text-center"><div className="flex justify-center gap-1">{editingRowKey === item.sampleKey ? <><button type="button" onClick={(event) => { event.stopPropagation(); setEditingRowKey(null); }} disabled={saveRow.isPending} className="ui-icon-button h-7 px-2 text-[10px] font-black">취소</button><button type="button" onClick={(event) => { event.stopPropagation(); saveRow.mutate(item); }} disabled={saveRow.isPending || !rowTitle.trim() || !rowKey.trim()} className="ui-icon-button-brand h-7 px-2 text-[10px] font-black disabled:opacity-50">저장</button></> : <><button type="button" onClick={(event) => { event.stopPropagation(); void copySampleApi(item); }} aria-label={item.title + " 샘플 조회 URL과 설명 복사"} title="GET 메서드, 전체 URL, 설명 복사" className="ui-icon-button size-7 text-brand-primary">{copiedSampleKey === item.sampleKey ? <Check className="size-3.5" /> : <Copy className="size-3.5" />}</button><button type="button" onClick={(event) => { event.stopPropagation(); preview(item); }} aria-label={item.title + " 미리 보기"} title="미리 보기" className="ui-icon-button size-7 text-brand-primary"><Eye className="size-3.5" /></button><button type="button" onClick={(event) => { event.stopPropagation(); beginRowEdit(item); }} className="ui-icon-button h-7 px-2 text-[10px] font-black">수정</button><button type="button" onClick={(event) => { event.stopPropagation(); if (window.confirm(`'${item.title}' 예제를 삭제할까요?`)) remove.mutate(item.sampleKey); }} disabled={remove.isPending} aria-label={item.title + " 샘플 삭제"} title="삭제" className="ui-icon-button size-7 text-destructive disabled:opacity-50"><Trash2 className="size-3.5" /></button></>}</div>{editingRowKey === item.sampleKey && saveRow.isError && <p role="alert" className="mt-1 text-[10px] font-bold text-destructive">저장 실패</p>}{remove.isError && <p role="alert" className="mt-1 text-[10px] font-bold text-destructive">삭제 실패</p>}</td>
              </tr>)}</tbody>
            </table>
          </div>
        )}
      </div>
      {showInlinePreview && <div className="min-h-0 p-4">
      ...
      {!showInlinePreview && previewDialogOpen && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-[140] flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label={editing ? "작성 샘플 수정" : "작성 샘플 미리 보기"} onMouseDown={() => setPreviewDialogOpen(false)}>
          <section className="flex h-[min(820px,calc(100vh-2rem))] w-[min(980px,calc(100vw-2rem))] flex-col overflow-hidden rounded-xl border border-surface-border bg-surface-raised shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            <header className="flex items-center justify-between gap-3 border-b border-surface-border-soft px-4 py-3">
              <div className="min-w-0 flex-1"><p className="text-[11px] font-black text-brand-primary">{editing ? "작성 샘플 수정" : "작성 샘플 미리 보기"}</p>{editing ? <div className="mt-1 grid gap-1.5 sm:grid-cols-2"><input value={title} onChange={(event) => setTitle(event.target.value)} aria-label="예제 제목" className="ui-input h-8 min-w-0 text-sm font-black" /><input value={sampleKeyDraft} onChange={(event) => setSampleKeyDraft(event.target.value.toUpperCase())} aria-label="예제 키" className="ui-input h-8 min-w-0 font-mono text-xs" /></div> : <h3 className="truncate text-sm font-black text-text-primary">{sample.data?.title ?? "샘플을 불러오는 중입니다."}</h3>}</div>
              <div className="flex shrink-0 items-center gap-1.5">{editing && <><button type="button" onClick={cancelEdit} disabled={save.isPending} className="ui-icon-button h-8 px-2.5 text-[11px] font-black">취소</button><button type="button" onClick={() => save.mutate()} disabled={save.isPending || !title.trim() || !sampleKeyDraft.trim()} className="ui-icon-button-brand h-8 px-2.5 text-[11px] font-black disabled:opacity-50">{save.isPending ? "저장 중" : "저장"}</button></>}<button type="button" onClick={() => setPreviewDialogOpen(false)} aria-label="미리 보기 닫기" className="ui-icon-button size-8"><X className="size-4" /></button></div>
            </header>
            <div className="min-h-0 flex-1 overflow-auto p-4">
              {save.isError && <p role="alert" className="mb-3 text-xs font-bold text-destructive">{save.error instanceof Error ? save.error.message : "예제를 저장하지 못했습니다."}</p>}
              {sample.isLoading ? <div className="grid h-full min-h-[320px] place-items-center text-sm font-bold text-text-muted">샘플 노트를 불러오는 중입니다.</div> : sample.isError || !sample.data ? <div role="alert" className="grid h-full min-h-[320px] place-items-center text-sm font-bold text-destructive">샘플 노트를 불러오지 못했습니다.</div> : <LexicalEditor key={editing ? `${sample.data.sampleKey}-editing-${sample.data.version}` : `${sample.data.sampleKey}-${sample.data.version}`} initialState={editing ? content : sample.data.content} onChange={editing ? setContent : () => undefined} readOnly={!editing} minHeight="520px" scrollable />}
            </div>
          </section>
        </div>,
        document.body,
