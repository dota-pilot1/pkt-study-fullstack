  ...
  return (
    <div className="flex min-h-0 flex-col">
      ...
      {!showInlinePreview && previewDialogOpen && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-[140] flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label="작성 샘플 미리 보기" onMouseDown={() => setPreviewDialogOpen(false)}>
          <section className="flex h-[min(820px,calc(100vh-2rem))] w-[min(980px,calc(100vw-2rem))] flex-col overflow-hidden rounded-xl border border-surface-border bg-surface-raised shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            <header className="flex items-center justify-between gap-3 border-b border-surface-border-soft px-4 py-3">
              <div className="min-w-0"><p className="text-[11px] font-black text-brand-primary">작성 샘플 미리 보기</p><h3 className="truncate text-sm font-black text-text-primary">{sample.data?.title ?? "샘플을 불러오는 중입니다."}</h3></div>
              <button type="button" onClick={() => setPreviewDialogOpen(false)} aria-label="미리 보기 닫기" className="ui-icon-button size-8"><X className="size-4" /></button>
            </header>
            <div className="min-h-0 flex-1 overflow-auto p-4">
              {sample.isLoading ? <div className="grid h-full min-h-[320px] place-items-center text-sm font-bold text-text-muted">샘플 노트를 불러오는 중입니다.</div> : sample.isError || !sample.data ? <div role="alert" className="grid h-full min-h-[320px] place-items-center text-sm font-bold text-destructive">샘플 노트를 불러오지 못했습니다.</div> : <LexicalEditor key={`${sample.data.sampleKey}-${sample.data.version}`} initialState={sample.data.content} onChange={() => undefined} readOnly minHeight="520px" scrollable />}
            </div>
          </section>
        </div>,
        document.body,
