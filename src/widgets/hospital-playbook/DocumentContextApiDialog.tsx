...
export default function DocumentContextApiDialog({ documentId, documentTitle, onClose }: DocumentContextApiDialogProps) {
  const { guide, tabs } = useMemo(() => {
    ...
    const tabs: ApiGuideTab[] = [
      ...
      { id: "response", label: "응답 구조", summary: "하위 문서는 children 안에 재귀적으로 포함됩니다.", content: `{"spaceCode":"PKT_FRONT_LEV1","topicId":23,"document":{"id":${documentId},"parentId":null,"title":"${documentTitle}","content":"Lexical JSON 문자열","version":1,"children":[{"id":11,"parentId":${documentId},"title":"하위 문서 제목","children":[]}]}}\n\n실제 응답의 나머지 메타데이터는 서버 응답을 기준으로 사용합니다.` },
