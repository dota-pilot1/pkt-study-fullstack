...
export function normalizeLexicalJson(value: string, promoteStructure = true): string | null {
  try {
    ...
    const children = Array.isArray(normalizedRoot.children) ? normalizedRoot.children : []
    // Lexical rejects an empty root. Bootstrap/imported documents may still
    // legitimately contain no blocks, so represent them with one paragraph.
