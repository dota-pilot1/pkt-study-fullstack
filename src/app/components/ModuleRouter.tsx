...
export function ModuleRouter({ children }: { children: React.ReactNode; tree?: unknown }) {
  ...
  if (moduleConfig) return <main className="flex min-h-0 min-w-0 flex-1"><HospitalPlaybookModule key={active} domain={moduleConfig.domain} title={moduleConfig.title} initialCategoryId={moduleConfig.initialCategoryId} initialTopicId={moduleConfig.initialTopicId} /></main>;
